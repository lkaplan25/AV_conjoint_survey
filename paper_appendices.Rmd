...
results_mnl <- make_coef_table(mnl_dummy_WTP)
results_mnl_weighted <- make_coef_table(mnl_dummy_WTP_weighted)

summary_final <- results_mxl %>%
  full_join(results_mxl_weighted, by = "coefficients") %>%
  full_join(results_mnl, by = "coefficients") %>%
  full_join(results_mnl_weighted, by = "coefficients") %>%
  mutate(
...
names(summary_final) <- c("coefficient", "MXL", "MXL Weighted", "MNL", "MNL Weighted")
...
modelResults_mnl_weighted <- round(summary(mnl_dummy_WTP_weighted)$statTable, 1)
...
modelResults_mnl_weighted$par <- row.names(modelResults_mnl_weighted)

modelResults_full <- modelResults_mxl %>%
  left_join(modelResults_mxl_weighted, by = "par") %>%
  left_join(modelResults_mnl, by = "par") %>%
  left_join(modelResults_mnl_weighted, by = "par") %>%
  select("par", ".x", ".y", ".x.x", ".y.y")

names(modelResults_full) <- c("Attribute", "MXL", "MXL weighted", "MNL", "MNL_Weighted")
...
bump_chart1 <- sens_price %>% 
  ...
  geom_dl(aes(label = mode), method = list(dl.trans(x = x + 0.5), "last.bumpup", cex = .75)) +
...
ggsave(
  filename = here('figs', 'scenarioPlotAll_appendix.png'), 
